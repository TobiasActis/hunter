...
def api_brain_trades():
    ...
    from core.market_brain import get_trades_snapshot
    return Response(content=json.dumps(get_trades_snapshot(), default=str).encode("utf-8"), media_type="application/json")
...
def _build_api_data() -> dict:
    with get_conn() as conn:
        ...
        VERSIONS = [
            ...
            ("v7", "v7 - + presión de venta solo después de 45 s", "sim_v7_since"),
        ]
        marks = {r["key"]: r["value"] for r in conn.execute(
            "SELECT key, value FROM dashboard_settings WHERE key IN ('sim_v2_since','sim_v3_since','sim_v4_since','sim_v5_since','sim_v6_since','sim_v7_since')")}
